...
export default function Menu() {
   ...
   * ===================================================
   *
   * This is only used to know how many database pages
   * exist.
   *
   * UI pagination is handled separately below.
   * ===================================================
   */

  const [apiPagination, setApiPagination] =
    useState<MenuPagination | null>(null);
  ...
  const debouncedSearchQuery =
    useDebounce(searchQuery, 350);

  const debouncedPriceRange =
  ...
  const [dollPosition, setDollPosition] =
    useState<{
      cardIndex: number;
      side: "left" | "right";
    } | null>(null);
   ...
   * API:
   *
   * /api/menu?page=1&limit=50
   * /api/menu?page=2&limit=50
   * ...
   *
   * All pages are combined into menuItems.
   *
   * This is necessary because filtering is currently
    ...
    const getAllMenuItems = async () => {
      try {
        ...
        while (true) {
          ...
          console.log(
            `Fetched menu page ${page}/${pagination.totalPages}: ${items.length} items`
          );
   ...
   * Safety:
   *
   * If filtering reduces the number of pages while
   * the user is on a later page, move them back.
   */
                                ...
                                className={`
                                  ...
                                  ${
                                    ...
                                    page
                                      ...
                                      : "bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 text-stone-600 dark:text-stone-300 hover:border-pink-300 hover:text-pink-600"
                                  }
                                `}
                              >
                                {
...
//           ? data.data
//           : [];
//           console.log('itemssss', items.length)
